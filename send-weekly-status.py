...
def _pretty_date_range(since: str, until: str) -> str:
    """e.g. 'May 4 — May 11, 2026'"""
    s = datetime.fromisoformat(since); u = datetime.fromisoformat(until)
    same_year = s.year == u.year
    if same_year:
        return f"{s.strftime('%b %-d')} – {u.strftime('%b %-d, %Y')}" if hasattr(s, "strftime") else f"{since} – {until}"
    return f"{s.strftime('%b %-d, %Y')} – {u.strftime('%b %-d, %Y')}"
